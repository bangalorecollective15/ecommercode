...
export default function SubcategoriesPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      let uploadedImageUrl = editingSub?.image_url || null;

      // Upload image if a new one is selected
      if (imageFile) {
        const fileExt = imageFile.name.split('.').pop();
        const fileName = `${Math.random()}.${fileExt}`;
        const filePath = `subs/${fileName}`;
        ...
        <div className="lg:col-span-4 sticky top-10">
          ...
          >
            ...
            <div className="space-y-6">
              {/* Image Upload UI */}
              <div className="group">
                <label className="text-[10px] uppercase tracking-[0.2em] font-black text-slate-400 mb-3 block ml-1">Visual Identity</label>
                <div 
                ...
                >
                  {imagePreview ? (
                    ...
                    <>
                      ...
                      <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest group-hover:text-[#2b2652]">Upload Asset</span>
                    </>
                  )}
                  {imagePreview && (
