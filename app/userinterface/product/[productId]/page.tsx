...
export default function ProductDetailsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] bg-[radial-gradient(at_top_right,_#e2e8f0,_#f1f5f9,_#ffffff)] pb-20 antialiased text-slate-900">
      ...
      <div className="max-w-8xl mx-auto px-4 lg:px-8 pt-40">
            ...
            <div className="flex flex-col py-4">
              <div className="space-y-8">
                <div className="space-y-4">
                  <nav className="inline-flex items-center gap-2 px-3 py-1 bg-black backdrop-blur-lg rounded-full border border-white/20 text-[10px] font-black uppercase tracking-widest text-white">
                  ...
                  <div className="flex items-center gap-6 pt-2">
                    <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400">
                      <span className="text-slate-900 underline underline-offset-4 decoration-orange-500">{product.brands?.name_en}</span>
                    </div>
                ...
                {/* ENHANCED GLASSMORPHIC PRICE SECTION */}
                <div className="relative group">
                  <div className="absolute -inset-4 bg-gradient-to-r from-orange-500/10 to-transparent blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                  <div className="relative flex items-center gap-6 border-y border-slate-200/40 py-8">
                    <div className="flex flex-col">
                      <div className="flex items-center gap-4">
                        <span className="text-6xl lg:text-7xl font-black text-slate-900 tracking-tighter">
                          ₹{selectedVar?.sale_price || selectedVar?.price}
                        </span>

                        {/* Savings Badge */}
                        {selectedVar?.sale_price && selectedVar.sale_price < selectedVar.price && (
                          <div className="bg-orange-600 text-white text-[10px] font-black px-3 py-1.5 rounded-full shadow-xl shadow-orange-200 animate-bounce">
                            {Math.round(((selectedVar.price - selectedVar.sale_price) / selectedVar.price) * 100)}% OFF
                          </div>
                        )}
                      </div>
                      ...
                      {selectedVar?.sale_price && selectedVar.sale_price < selectedVar.price && (
                        <span className="text-2xl text-slate-300 line-through font-bold tracking-tight ml-1 decoration-slate-400/50">
                          M.R.P: ₹{selectedVar?.price}
                        </span>
                      )}
                    </div>

                    {/* Urgent Stock Badge */}
                    {selectedVar?.stock > 0 && selectedVar?.stock < 5 && (
                      <div className="ml-auto px-4 py-2 bg-red-50 border border-red-100 rounded-2xl">
                        <p className="text-[10px] font-black text-red-600 uppercase tracking-tighter">Only {selectedVar.stock} left</p>
                      </div>
                    )}
                  </div>
                </div>

                {/* Variation Selection (Modern Chips) */}
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Select Variation</p>
                    <button className="text-[9px] font-bold uppercase tracking-widest text-blue-600 hover:underline">Size Guide</button>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    {variations.map((v) => (
                      <button
                        ...
                        className={`px-6 py-4 rounded-[24px] border-2 transition-all text-left min-w-[140px] relative overflow-hidden ${selectedVar?.id === v.id
                            ? "bg-slate-900 border-slate-900 text-white shadow-2xl scale-[1.05]"
                            : "bg-white/40 backdrop-blur-md border-white/60 text-slate-700 hover:border-slate-300"
                          }`}
                      >
                        <div className="flex flex-col">
                          <span className={`text-[8px] font-black uppercase tracking-[0.2em] mb-1 ${selectedVar?.id === v.id ? "text-orange-400" : "text-slate-400"}`}>
                            {v.color?.name || 'Standard'}
                          </span>
                          <span className="text-sm font-black italic">{v.size?.name || 'OS'}</span>
                        </div>
                        {selectedVar?.id === v.id && (
                ...
                {/* Rest of Action Controls (Quantity, Add to Bag, etc.) */}
