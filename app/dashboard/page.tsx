        ...
        <div className="p-4 md:p-10 space-y-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <DashboardCard
              ...
              value={`₹${stats.onlineSales.toLocaleString()}`}
            ...
            <DashboardCard
              title="POS Revenue"
              value={`₹${stats.posSales.toLocaleString()}`}
...
function DashboardCard({ title, value, icon, gold, link }: any) {
  const router = useRouter();

  return (
    <div
      onClick={() => link && router.push(link)}
      className={`bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-50 flex flex-col justify-between group hover:border-brand-gold/50 transition-all duration-500 shadow-sm hover:shadow-2xl hover:shadow-brand-blue/5 relative overflow-hidden cursor-pointer active:scale-[0.99]`}
    >
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <ArrowUpRight size={16} className="text-slate-200 group-hover:text-brand-gold transition-colors duration-500 transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
      </div>
