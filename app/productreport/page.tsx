...
export default function ProductReport() {
  ...
  >("created_at");
  // 1. Added a new filter state for inventory tiers
  ...
  // 2. Updated Search, Sort, and Stock level processing filter
  const handleSearchAndSort = () => {
    ...
    // Process structural stock filters
    if (stockFilter === "low") {
      // Items with stock greater than 0 but strictly lower than 10
      temp = temp.filter(p => (p.stock || 0) > 0 && (p.stock || 0) < 10);
  ...
  // 3. Keep tracking of stockFilter adjustments inside listener hooks
  useEffect(() => { handleSearchAndSort(); }, [search, sortBy, stockFilter, products]);
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center bg-white p-4 rounded-[1.8rem] border border-slate-100 shadow-sm">
          ...
          <div className="lg:col-span-7 flex flex-col sm:flex-row gap-3">
            {/* 4. Added Inventory Level Status Filter Selection */}
            <div className="relative flex-1">
              <Layers className="absolute left-4 top-1/2 -translate-y-1/2 text-[#c4a174]" size={16} />
              <select
                value={stockFilter}
              ...
              >
                <option value="all">All Inventory Volumes</option>
            ...
            <button
              onClick={() => { setSearch(""); setSortBy("created_at"); setStockFilter("all"); }}
              className="w-14 h-14 flex items-center justify-center bg-slate-100 text-[#2b2652] rounded-xl hover:bg-[#c4a174] transition-all group shrink-0"
            >
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                ...
                {paginatedItems.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-10 py-16 text-center text-[11px] font-black uppercase tracking-widest text-slate-400">
                      No inventory matched these filters.
                    </td>
                  </tr>
                )}
              </tbody>
