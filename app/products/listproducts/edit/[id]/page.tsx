...
export default function EditLifestyleProduct({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleCategoryChange = async (categoryId: string) => {
    ...
    setSubSubcategories([]); // Clear sub-sub child tree
  ...
  const handleUpdate = async () => {
    ...
    try {
      // 1. Update Core Product (Now syncing SKU changes)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-16">
          <div className="lg:col-span-3 space-y-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              {/* EDITABLE SKU COMPONENT */}
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-[#c4a174] tracking-[0.2em]">Global SKU (Primary Key)</label>
                <div className="flex items-center gap-3 py-4 border-b-2 border-slate-100 text-[#2b2652] focus-within:border-[#2b2652] transition-colors">
                  <Edit2 size={14} className="text-slate-400" />
                  <input 
                    className="w-full bg-transparent font-mono text-base font-bold outline-none uppercase" 
                    value={form.sku} 
            ...
            <div className="space-y-8 bg-white p-8 rounded-[2.5rem] border border-slate-50 shadow-xl shadow-[#2b2652]/5">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4 pt-4 border-t border-slate-50">
                  ...
                  {/* TIER 3 SUB-SUBCATEGORY DROPDOWN */}
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-[#2b2652] uppercase tracking-widest block opacity-40">Tier 3 Sub-Sub</label>
                    <select 
                      className="w-full p-4 bg-[#FBFBFC] border border-slate-100 rounded-xl text-[11px] font-black uppercase outline-none focus:border-[#c4a174]" 
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {form.existingImages.map((img) => {
                  ...
                  return (
                    <div
                      key={img.id}
                      className="relative aspect-square rounded-[1.2rem] overflow-hidden border border-slate-50 group bg-black"
                    >
                      {isVideo ? (
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 bg-white/90 backdrop-blur shadow-md text-red-500 p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-all z-10"
                      >
                        <X size={12} />
                      </button>
                    </div>
