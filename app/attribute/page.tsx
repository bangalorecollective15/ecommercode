...
export default function LifestyleAttributes() {
  ...
  const updateAttribute = async (id: number) => {
...
const deleteAttribute = async (attr: Attribute) => {
    try {
      const { error } = await supabase.from("attributes").delete().eq("id", attr.id);
      if (error) throw error;
              ...
              <div key={attr.id} className="group/item flex justify-between items-center bg-slate-50/50 hover:bg-[#2b2652] px-5 py-4 rounded-[1.5rem] transition-all duration-300">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  {editingId === attr.id ? (
                    <input 
                      autoFocus
                      className="bg-transparent border-b border-[#c4a174] outline-none text-[11px] font-black uppercase text-white w-full"
                      value={editValue}
                      onChange={(e) => setEditValue(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && updateAttribute(attr.id)}
                    />
                  ) : (
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover/item:opacity-100 transition-all">
                  {editingId === attr.id ? (
                    <button onClick={() => updateAttribute(attr.id)} className="text-green-400 hover:text-green-300 scale-90 hover:scale-110">
                      <Check size={15} />
                    </button>
                  ) : (
                    <button 
                      onClick={() => { setEditingId(attr.id); setEditValue(attr.name); }}
                      className="text-slate-300 hover:text-[#c4a174] transition-all scale-90 hover:scale-110"
                    >
                      <Pencil size={15} />
                    </button>
                  )}
               <button 
                    ...
                    className="text-slate-300 hover:text-red-400 transition-all scale-90 hover:scale-110"
                  >
                    <Trash2 size={15} />
                  </button>
                </div>
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFC] p-6 md:p-12 text-[#2b2652] selection:bg-[#c4a174] selection:text-white font-sans">
      <Toaster position="bottom-right" toastOptions={{ style: { background: '#2b2652', color: '#c4a174', fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.1em', borderRadius: '20px', padding: '16px 24px', border: '1px solid rgba(196, 161, 116, 0.2)' } }} />
