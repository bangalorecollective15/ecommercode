...
export default function BrandList() {
  ...
  // Hidden file input reference for easier clicks
  ...
  const handleSave = async () => {
    ...
    // Clean up old storage reference if image explicitly removed or swapped
    if (requireNewImage && selectedBrand.image_url) {
      const path = selectedBrand.image_url.split("/").pop()?.split("?")[0];
      if (path) {
        try {
          await supabase.storage.from("brand-images").remove([path]);
        } catch (err) {
          console.error("Failed to delete older asset:", err);
        }
      }
          ...
          <div className="bg-white border border-white/20 rounded-[3rem] w-full max-w-xl shadow-[0_30px_100px_-20px_rgba(43,38,82,0.3)] overflow-hidden animate-in fade-in zoom-in duration-500">
            ...
            <div className="p-12 space-y-10">
              <div className="flex flex-col items-center gap-4">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Identity Visual (Click to Change)</label>
                
                {/* Hidden File Input accessed natively via reference trigger */}
                <input 
                  type="file"
                  ref={fileInputRef}
                ...
                >
                  <div className="w-44 h-44 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center overflow-hidden bg-slate-50 group-hover:border-[#c4a174] transition-all duration-500 shadow-inner p-4">
                    {imageFile ? (
                      ...
                      <img src={selectedBrand.image_url} className="w-full h-full object-contain" alt="Current" />
                    ) : (
                      <div className="flex flex-col items-center gap-3 text-center">
                        <Upload className="w-7 h-7 text-slate-300 group-hover:text-[#c4a174] transition-colors" />
                        <span className="text-[9px] font-black uppercase text-slate-400 tracking-[0.15em]">Upload Source</span>
                      </div>
                    )}

                    {/* Overlay interaction prompt on hover */}
                    <div className="absolute inset-0 bg-[#2b2652]/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-[2.5rem]">
                      <span className="text-[10px] text-[#c4a174] font-black uppercase tracking-widest">Swap Visual</span>
                    </div>
                  </div>
                  
                  {/* Immediate reset functionality */}
                  {((selectedBrand.image_url && !requireNewImage) || imageFile) && (
                    <button
                      onClick={handleRemoveImage}
                      className="absolute -top-2 -right-2 bg-[#2b2652] text-[#c4a174] w-9 h-9 rounded-xl flex items-center justify-center shadow-2xl hover:bg-red-500 hover:text-white transition-all active:scale-90 z-10"
                      title="Clear image"
                    >
                      <X className="w-4 h-4" />
                    </button>
